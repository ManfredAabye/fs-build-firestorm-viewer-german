...
void LLFloaterModelPreview::loadModel(S32 lod, const std::string& file_name, bool force_disable_slm)
{
    ...
    std::string ext = gDirUtilp->getExtension(file_name);
    LLStringUtil::toLower(ext);
    std::string filename = file_name;

    if (ext != "dae")
    {
        std::string temp_dae = gDirUtilp->getTempFilename() + "_converted.dae";
        ...
        if (!scene)
        {
            LL_WARNS() << "Assimp failed to load " << filename << ": " << importer.GetErrorString() << LL_ENDL;
            return;
        }

        Assimp::Exporter exporter;
        if (exporter.Export(scene, "collada", temp_dae) != AI_SUCCESS)
        {
            LL_WARNS() << "Assimp failed to export to DAE: " << exporter.GetErrorString() << LL_ENDL;
            return;
        }
    ...
    mModelPreview->loadModel(filename, lod, force_disable_slm);
}
