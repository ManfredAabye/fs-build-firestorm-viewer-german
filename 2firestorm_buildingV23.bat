...
:: ohne avx2
@REM echo %GREEN%Konfiguration ohne AVX2...%RESET%
@REM autobuild configure --config-file "%AUTO_BUILD_CONFIG%" -A 64 -c ReleaseFS_open -- --package --chan WebRTC -DLL_TESTS:BOOL=FALSE -DFMOD:BOOL=OFF --openal
:: mit avx2
echo %GREEN%Konfiguration mit AVX2...%RESET%
@REM autobuild configure --config-file "%AUTO_BUILD_CONFIG%" -A 64 -c ReleaseFS_open -- --avx2 --package --chan WebRTC -DLL_TESTS:BOOL=FALSE -DFMOD:BOOL=OFF --openal
::autobuild configure --config-file "%AUTO_BUILD_CONFIG%" -A 64 -c ReleaseFS_open -- --avx2 --package --chan WebRTC -DLL_TESTS:BOOL=FALSE -DFMOD:BOOL=OFF --USE_OPENAL
::autobuild configure --config-file "%AUTO_BUILD_CONFIG%" -A 64 -c ReleaseFS_open -- --avx2 --package --chan WebRTC -DLL_TESTS:BOOL=FALSE -DFMOD:BOOL=OFF -DUSE_OPENAL=ON
